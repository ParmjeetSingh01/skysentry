...
def run_yolo(img,conf=0.18):
    ...
    for b in res.boxes:
        ...
        out.append({"bbox":[x1,y1,x2,y2],"conf":round(float(b.conf[0]),3),
                    ...
                    "is_threat":True,"center":[(x1+x2)//2,(y1+y2)//2]})
    return out
...
def annotate(img,dets):
    ...
    cv2.putText(img,"SkySentry AI",(10,img.shape[0]-12),cv2.FONT_HERSHEY_SIMPLEX,.5,(0,200,80),1)
    return img
...
@app.get("/")
def root():
    return{"service":"SkySentry","model_loaded":_model is not None,"docs":"/docs"}
...
async def detect_video(file:UploadFile=File(...),conf:float=0.18,every_n:int=5):
    ...
    cap.release()
    return{"filename":file.filename,"total_frames":total,"analyzed_frames":len(frames),
           "fps":fps,"total_detections":sum(r["count"] for r in frames),"frames":frames}
...
def icp(ip,tp,tv,spd=14.0):
    ...
    return{"heading_deg":round(math.degrees(math.atan2(-dy,dx))%360,1),
           ...
           "tti":round(tg,2),"feasible":spd>math.hypot(tvx,tvy)*.85,"distance":round(d,1)}
...
def tscore(trk,ip):
    ...
    return round(.35*prox+.25*ss+.25*ap+.15*(1-min(math.hypot(px-ip[0],py-ip[1])/800,1)),4)
...
class Sess:
    ...
    def step(self):
        ...
        for t in tracks:
            s=tscore(t,(self.ix,self.iy));i=icp((self.ix,self.iy),t["position"],t["velocity"])
