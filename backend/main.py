...
REMAP = {
    ...
    "cell phone":"comms-device","laptop":"command-terminal","umbrella":"signal-blocker",
    "suitcase":"payload-case","dog":"recon-animal","cat":"recon-animal",
    "boat":"watercraft-threat","bus":"troop-carrier","train":"rail-vehicle",
}
...
async def detect_image(file: UploadFile = File(...), conf: float = 0.18):
    ...
    return {"filename":file.filename,"shape":list(img.shape[:2]),
            "detections":dets,"count":len(dets),"aerial_count":len(aerial),
...
async def detect_video(file: UploadFile = File(...), conf: float = 0.18, every_n: int = 5):
    ...
    while True:
        ...
        if fi%every_n==0:
            ...
            frames.append({"frame_idx":fi,"timestamp_s":round(fi/fps,3),
                           "detections":dets,"count":len(dets)})
        fi+=1
    cap.release()
    return {"filename":file.filename,"total_frames":total,
            "analyzed_frames":len(frames),"fps":fps,
            "total_detections":sum(r["count"] for r in frames),"frames":frames}
...
def icp(ip,tp,tv,spd=14.0):
    ...
    return{"heading_deg":round(math.degrees(math.atan2(-dy,dx))%360,1),
           ...
           "tti":round(tg,2),"feasible":spd>math.hypot(tvx,tvy)*0.85,"distance":round(d,1)}
...
class SD:
    _c=0; TYPES=["quadcopter","fixed-wing","hexcopter"]
    def __init__(self):
...
class Sess:
    ...
    def step(self)->Dict:
        ...
        return{"frame":self.frame,"ts":int(time.time()*1000),"arena":{"w":W,"h":H},
               ...
               "target":bt,"stats":self.stats,"status":"THREAT DETECTED" if tgts else "ALL CLEAR"}
...
async def ws_sim(ws:WebSocket):
    ...
    try:
        while True:
            ...
            await ws.send_text(json.dumps(s.step())); await asyncio.sleep(1/30)
    except WebSocketDisconnect: pass

@app.get("/health")
def health():
    return{"status":"ok","version":"3.0.0","model":"yolov8n","model_loaded":_model is not None}

@app.get("/")
def root():
    return{"service":"SkySentry API",
           "endpoints":["/detect/image","/detect/video","/ws/webcam","/ws/sim","/docs"]}
